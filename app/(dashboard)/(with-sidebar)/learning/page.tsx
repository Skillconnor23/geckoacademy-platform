...
export default async function LearningPage() {
  ...
  const [quizzes, stats, assessments] = await Promise.all([
    getQuizzesForStudentClasses(user.id),
    getStudentDashboardStats(user.id),
    getQuizResultsForStudentProfile(user.id),
  ]);
  ...
  return (
    <section className="flex-1 p-4 lg:p-8">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        Weekly quizzes from your class.
      </p>
      ...
      {latest && (
        <>
          <Card
            className={`mb-4 rounded-2xl border-2 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] ${
                ...
                {quizzes.map(({ quiz, className, submission }) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-[#1f2937] truncate text-sm">
                        {quiz.title}
                      </p>
